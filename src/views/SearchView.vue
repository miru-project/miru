...
const search = async (pkg: string) => {
  ...
  try {
    console.log('搜索了', pkg);
    data.length = 0
    const res = await miru.expands.get(pkg).search(kw.value, page.value) as never[] ?? []
    res.forEach((e: any) => {
      e.pkg = pkg
    })
    data.push(...res)
...
<template>
  <main>
    ...
    <div v-if="miru.expands.size">
      <div class="switch">
        <button v-for="(v, k) in miru.expands" :class="{ 'activit': activeExpand == v[0] }"
      ...
      <h3 v-if="!kw">最近更新</h3>
      <div>
        <GridList v-if="miru.getNameforPackge(activeExpand) ?? false" :list="data" :loading="loading" :nodata="nodata">
        </GridList>
      </div>
    </div>
    <div v-else>
